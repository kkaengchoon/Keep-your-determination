...
def logout():
    try:
        ...
        # URL 파라미터 변경을 통해 새로고침 효과
        st.experimental_set_query_params()  # Query parameters를 초기화하여 새로고침 효과
    except Exception as e:
    ...
    with st.expander("새 일정 추가"):
        ...
        if st.button("일정 추가"):
            try:
                ...
                add_event(service, event_summary, "", "", start_datetime, end_datetime)
        ...
        if events:
            ...
            if st.button("일정 수정"):
                try:
                    ...
                    new_start_datetime = datetime.combine(date.today(), start_time)
                    new_end_datetime = datetime.combine(date.today(), end_time)
